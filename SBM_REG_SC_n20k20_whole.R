...
whole.run <- function(i)
{
  ...
  net <- SBM.generate(N = 20000, k = 20, 
                      ...
                      ncore = 20)
  time <- system.time(
    out <- reg.spectral.cluster(A = net$A, k = 20))[3]
  ...
  write.table(data.frame(time = as.numeric(tmp[1]), error = tmp[2]),
            '~/revision/output/sonnet/n20k20/reg/TEMP_SBM_REG_SC_n20k20_whole.csv',
...
write.table(tab, '~/revision/output/sonnet/n20k20/reg/SBM_REG_SC_n20k20_whole.csv',
