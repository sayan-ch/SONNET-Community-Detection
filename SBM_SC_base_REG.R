...
reg.sp.cl <- function(A, k, degree)
{
  ...
  i <- which(is.na(E1), T)
  if(nrow(i) == 0){
  message('nrow(i) = 0 - kmeans start')
   return(as.integer(
    kmeans(E1, k, 100000000, 30)$cluster
        ))}
  
  message('nrow(i) != 0 - removval step')
  i <- unique(i[,1])
  ...
  E2 <- E1[-i,]
  message('kmeans start')
  out[-i] <-  as.integer(
    kmeans(E2, k, 100000000, 30)$cluster
    )
  message('kmeans done')
...
reg.spectral.cluster <- function(A, k)
{
  message('find 0')
  ...
  l <- (lab==0)
  message('find 0 done - remaining matrix start')
  print(paste0('Num deg 0 nodes = ', sum(index)) )
  # perform spectral clustering on the remaining matrix
  lab[l] <- reg.sp.cl(A[l, l], k, degree[l])
  return(lab)
}
...
#############################################################
son_sample <- function(A, N, s, O, m, rep, 
                       cut.over = 0.95, cut.rep = 0.9*cut.over)
{
  on.exit(gc())
  while(1){
    print(c(N,O))
    over <- sample(1:N, O, F)
    print(length(over))
    non1 <- sample((1:N)[-over], N-O, replace = F)
    de <- vector()
    for(pi in 1:s)
      de[pi] <- sum(colSums(A[c(over, non1[((pi-1)*m+1):(pi*m)]),
                              c(over, non1[((pi-1)*m+1):(pi*m)])])!=0) >= floor((O+m)*cut.over)
    if(all(de)) break
  }
  
  if(rep == 0) return(list(over = over,
                           non = cbind(non1)))
  
  non <- matrix(nrow = N-O, ncol = rep)
  re <- 1
  while(re <= rep) {
    print(2)
    trial <- sample((1:N)[-over], N-O, replace = F)
    de <- vector()
    for(pi in 1:s)
      de[pi] <- sum(colSums(A[trial[((pi-1)*m+1):(pi*m)],
                              trial[((pi-1)*m+1):(pi*m)]])!=0) >= floor(m*cut.rep)
    if(all(de)) {
      non[,re] <- trial
      re <- re+1
    }
  ...
  return(list(over = over,
              non = cbind(non1, non)))
}

sparseSONNET <- function(A, k, s, O, rep = 0, cut.over, ncore = 1)
{
  on.exit(gc())
  ...
  output <- sapply(mclapply(1:(s*(rep+1)), function(i){
    ...
    if(r != 1)  
    {
      label[index] <- reg.spectral.cluster(A[index, index], k)
      return(label)
    }else{
      label[c(over,index)] <- reg.spectral.cluster(A[c(over, index),
                                                     c(over, index)], k)
      return(label)
    }
  }, mc.cores = ncore), 'c')
